...
import type { InitOptions, ProjectConfig, TemplateRegistry } from "../types.js";
import {
  ...
  try {
    ...
    if (!options.force && (await isProjectInitialized())) {
      Logger.errorBox(
        ...
        "This directory already contains a mycli project.\nUse --force to reinitialize."
      );
      process.exit(1);
    }
    ...
    Logger.info(
      `Initializing mycli project: ${Logger.bold("blue", projectName)}`
    );
    ...
    Logger.successBox(
      ...
      `Project "${projectName}" has been initialized with:\n\n` +
